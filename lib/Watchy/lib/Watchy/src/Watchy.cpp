  ...
  if (url != "") {
    if (connectWiFi()) {
      ...
      if (httpResponseCode == 200) {
        ...
        currentWeather.sunvisible = int(responseObject["current"]["dt"]) > int(responseObject["current"]["sunrise"]) && int(responseObject["current"]["dt"]) < int(responseObject["current"]["sunrise"]);
      } else {
        // http error
      }
      http.end();
