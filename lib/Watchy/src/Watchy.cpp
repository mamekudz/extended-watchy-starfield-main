...
#endif
  // Serial.println("Gehe in den Deep Sleep...");
  // Serial.flush();
  // delay(200);
...
weatherData Watchy::getWeatherData(String url) {
  ...
  if (url != "") {
    if (connectWiFi()) {
      ...
      if (httpResponseCode == 200) {
        ...
        currentWeather.temperature = int(responseObject["current"]["temp"]);
        currentWeather.weatherConditionCode = int(responseObject["current"]["weather"][0]["id"]);
        currentWeather.sunvisible = int(responseObject["current"]["dt"]) > int(responseObject["current"]["sunrise"]) && int(responseObject["current"]["dt"]) < int(responseObject["current"]["sunset"]);
        Serial.print(currentWeather.sunvisible ? "sun visible": "sun not visible");
      } else {
        // http error
      }
      http.end();
      ...
      currentWeather.sunvisible = true;
    }
  }
