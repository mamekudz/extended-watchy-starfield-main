...
float Dusk2Dawn::getSolarDeclination(int y, int m, int d) {
  // Ruft die privaten Hilfsfunktionen auf, um die Deklination zu berechnen.
  float jday = jDay(y, m, d);
  float t = fractionOfCentury(jday);
  return sunDeclination(t);
}

bool Dusk2Dawn::isPolarWinter(int y, int m, int d, bool isDST) {
  // 1. Prüfung auf Polarzeit: Keiner der Punkte darf existieren (-1)
  int sunrise_time = sunrise(y, m, d, isDST);
  int sunset_time = sunset(y, m, d, isDST);
...
int Dusk2Dawn::getSolarNoonTime(int y, int m, int d, bool isDST) {
  float jday = jDay(y, m, d);
  float t = fractionOfCentury(jday);
  float E = equationOfTime(t);
  float noon_utc_minutes = 720.0 - (4.0 * _longitude) - E;
  float total_offset_minutes = _timezone * 60.0;
  if (isDST) {
...
int Dusk2Dawn::getSolarMidnightTime(int y, int m, int d, bool isDST) {
  int noon_time = getSolarNoonTime(y, m, d, isDST);
